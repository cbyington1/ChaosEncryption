...
# Set parameters for the Rössler system
a = float(sys.argv[1])
b = 0.2
c = float(sys.argv[2])
...
def compute_chaotic_values(_):
    ...
    sol = solve_ivp(rossler_system, t_span, initial_values, args=(a, b, c), t_eval=np.linspace(0, t_span[1], 1000))
...
if __name__ == '__main__':
    ...
    file_path = os.path.join("chaos_coordinates", 'rossler_values.txt')
    with open(file_path, 'w') as file:
        for x, y, z in zip(x_values, y_values, z_values):
            file.write(f"{x} {y} {z}\n")

    # Create a 3D plot of the Rössler attractor (if needed)
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(x_values, y_values, z_values, lw=0.5)
    ax.set_title("Rössler Attractor")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")

    # Save the generated plot as an image
    image_path = os.path.join("chaos_graphs", 'rossler_system.png')
